...
// URLs para as épocas passadas (2024/25) - Testando com IDs conhecidos
const HISTORICAL_SEASON_URLS = {
  'laliga': 'https://stats.fn.sportradar.com/betano/pt/Europe:London/gismo/stats_season_fixtures2/118691', // La Liga 24/25
};
...
// Buscar dados históricos da SportsRadar para uma liga específica
...
async function saveHistoricalData(league, season = '2024-25') {
  try {
    ...
    // Processar dados dos jogos (similar ao dataLoader atual)
    const teamsData = {};
    ...
    if (seasonData.doc && seasonData.doc[0] && seasonData.doc[0].data && seasonData.doc[0].data.matches) {
      const matches = seasonData.doc[0].data.matches;
        ...
        // Corrigir parsing de data - formato DD/MM/YY da API
        let matchDate;
        if (match.time && match.time.date) {
          const dateStr = match.time.date;
          
          // Converter DD/MM/YY para formato reconhecido
          if (dateStr.includes('/')) {
            const [day, month, year] = dateStr.split('/');
            // Assumir que anos com 2 dígitos são 20XX
            const fullYear = year.length === 2 ? `20${year}` : year;
            matchDate = new Date(`${fullYear}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`);
          } else {
            matchDate = new Date(dateStr);
          }
          
          if (isNaN(matchDate.getTime())) {
            console.log(`⚠️  Data inválida para jogo ${homeTeam} vs ${awayTeam}: ${dateStr}`);
            return; // Pular este jogo se a data for inválida
          }
        } else {
          console.log(`⚠️  Sem data para jogo ${homeTeam} vs ${awayTeam}`);
          return; // Pular este jogo se não houver data
        }
        
        const matchTime = match.time.time || '00:00';        // Determinar status do jogo
        let status = 'finished'; // Épocas passadas = jogos terminados
        if (match.result && match.result.current) {
          status = 'finished';
        }
        ...
        if (!isTargetMatch) {
          return; // Pular jogos que não envolvem nossas equipas alvo
        }
        ...
        if (TEAMS.includes(homeTeam)) {
          ...
          teamsData[homeTeam].games.push({
            ...
            time: matchTime,
            status: status,
            teamScore: match.result?.current?.home || null,
            opponentScore: match.result?.current?.away || null,
            sportRadarId: match.id || `${homeTeam}_${awayTeam}_${matchDate.getTime()}`
          });
        }
        ...
        if (TEAMS.includes(awayTeam)) {
          ...
          teamsData[awayTeam].games.push({
            ...
            time: matchTime,
            status: status,
            teamScore: match.result?.current?.away || null,
            opponentScore: match.result?.current?.home || null,
            sportRadarId: match.id || `${homeTeam}_${awayTeam}_${matchDate.getTime()}`
          });
        }
      });
